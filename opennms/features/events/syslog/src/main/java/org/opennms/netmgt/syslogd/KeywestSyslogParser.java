...
public class KeywestSyslogParser extends SyslogParser {
    ...
    protected SyslogMessage parse() throws SyslogParserException {
        if (!this.find()) {
            if (traceEnabled()) {
                LOG.trace("'{}' did not match '{}', falling back to the custom parser", m_keywestPattern, getText());
       ...
       // message.setProcessId(matcher.group(5));
        message.setMessage(matcher.group(5).trim());
